# Muhammad Hussain <img align="right" src="https://komarev.com/ghpvc/?username=mh-husssain&label=Profile%20views&color=0e75b6&style=flat" alt="Profile views" />

**BSc (Hons) Computer Science with Industrial Experience — Queen Mary University of London**

I'm a software engineer focused on building reliable, well-structured systems across backend, full-stack, and cloud, with research interests in machine-learning interpretability and distributed systems. I enjoy taking projects from initial concept through to a finished, production-ready product.

## What I work on

- **Backend & API**
- **Full-stack web**
- **Cloud & DevOps**
- **AI & Machine Learning**

## Technical Skills

**Languages**

![Languages](https://skillicons.dev/icons?i=py,java,cpp,cs,swift,js,ts,php,html,css,bash)


**Frameworks & Libraries**

![Frameworks](https://skillicons.dev/icons?i=dotnet,django,fastapi,react,nodejs,jquery)


**Databases**

![Databases](https://skillicons.dev/icons?i=postgres,mysql,sqlite,redis)


**Cloud, Infrastructure & DevOps**

![Infrastructure](https://skillicons.dev/icons?i=gcp,docker,kubernetes,terraform)


**Tools & IDEs**

![Tools](https://skillicons.dev/icons?i=git,github,gitlab,vscode,visualstudio,idea,figma,postman,powershell)


**AI & Machine Learning**

![Machine Learning](https://skillicons.dev/icons?i=pytorch)


**Operating Systems**

![Operating Systems](https://skillicons.dev/icons?i=linux,windows,apple)


**Productivity & IT Ops**

`ServiceNow` · `Confluence` · `Power Automate` · `Excel` · `Teams` · `Slack`

<br>

## Contact

[![Gmail](https://skillicons.dev/icons?i=gmail)](mailto:inquiries.muhammadhishamhussain@gmail.com)
[![LinkedIn](https://skillicons.dev/icons?i=linkedin)](https://www.linkedin.com/in/muhammad-h-hussain/)
